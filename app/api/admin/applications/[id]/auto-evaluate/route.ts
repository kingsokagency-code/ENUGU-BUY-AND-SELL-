/**
 * app/api/admin/applications/[id]/auto-evaluate/route.ts
 * Admin endpoint to run Intelligent AI Candidate Evaluation on demand
 */

import { NextResponse } from 'next/server';
import { cookies } from 'next/headers';
import { getTeamApplicationById, evaluateStage1Application } from '@/lib/recruitment';
...
export async function POST(
  ...
  try {
    ...
    // Auto-update Stage 1 scores if currently unscored (or save as AI suggested breakdown)
    const updateRes = await evaluateStage1Application(id, {
    ...
    return NextResponse.json({
      success: true,
      ai_result: aiResult,
      application: updateRes.application || app,
    });
  } catch (err: unknown) {
    return NextResponse.json(
      { error: sanitizeErrorMessage(err, 'Failed to run AI candidate evaluation') },
      { status: 500 }
    );
  }
}
